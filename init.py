...
if __name__ == "__main__":
	...
	while on == True:
		...
		Spe_Text.listen_keyword(global_config["activation_word"])
		...
		while attempts <3 and success == False:
			...
			if heard_command == "hi":
				...
				heard_command = voice_question(dialogs["hi"]["d_1"])
				#success = True
			...
			if heard_command == "emergency":
				...
				if "heart" in kind_of_emergency and "attack" in kind_of_emergency:
					...
					compressionPath = os.path.join(os.getcwd(), "audio/compresion.mp3")
					os.system(f"play {compressionPath}")
				else:
